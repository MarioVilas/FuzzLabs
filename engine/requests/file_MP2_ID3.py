...
if s_block_start("MP2_FILE"):
    ...
    if s_block_start("MP2_FRAME_2"):
        s_binary("\xFF\xFD\x80\x04")
        s_binary("\x37" * 413)              # MPEG Audio Data
    s_block_end("MP2_FRAME_2")

    if s_block_start("MP2_FRAME_3"):
        s_binary("\xFF\xFD\x80\x04")
        s_binary("\x37" * 413)              # MPEG Audio Data
    s_block_end("MP2_FRAME_3")

    if s_block_start("MP2_FRAME_4"):
        s_binary("\xFF\xFD\x80\x04")
        s_binary("\x37" * 413)              # MPEG Audio Data
    s_block_end("MP2_FRAME_4")

    if s_block_start("MP2_FRAME_5"):
        s_binary("\xFF\xFD\x80\x04")
        s_binary("\x37" * 413)              # MPEG Audio Data
    s_block_end("MP2_FRAME_5")

    if s_block_start("MP2_FRAME_6"):
        s_binary("\xFF\xFD\x80\x04")
        s_binary("\x37" * 413)              # MPEG Audio Data
    s_block_end("MP2_FRAME_6")
    ...
    if s_block_start("MPEG_AUDIO_TAG"):
        s_static("TAG")
        s_string("FuzzLabs MP2 Fuzzer")         # Title
        s_binary("\x00" * 11)
