...
def s_checksum (block_name, algorithm="crc32", length=0, endian="<", name=None):
    ...
    if block_name in blocks.CURRENT.block_stack:
        raise sex.SullyRuntimeError("CAN NOT ADD A CHECKSUM FOR A BLOCK CURRENTLY IN THE STACK")
...
def s_size (block_name, offset=0, length=4, endian="<", format="binary", synchsafe=False, inclusive=False, signed=False, math=None, fuzzable=False, name=None):
    ...
    size = blocks.size(block_name, blocks.CURRENT, offset, length, endian, format, synchsafe, inclusive, signed, math, fuzzable, name)
...
def s_bitfield (value, length, fields=[], fuzzable=True, name=None):
    '''
    Create a bitfield which defines a value as fields of bits.

    @type  block_name: String
    @param block_name: Name of the bitfield block
    @type  fields:     List
...
def s_string (value, size=-1, padding="\x00", encoding="ascii", compression=None, fuzzable=True, max_len=0, name=None):
    ...
    @param compression: (Optonal, def=None) String compression, ex: zlib for zlib compression.
    @type  fuzzable: Boolean
    @param fuzzable: (Optional, def=True) Enable/disable fuzzing of this primitive
    ...
    s = primitives.string(value, size, padding, encoding, compression, fuzzable, max_len, name)
...
def s_dword (value, endian="<", format="binary", synchsafe=False, signed=False, full_range=False, fuzzable=True, name=None):
    '''
    ...
    dword = primitives.dword(value, endian, format, synchsafe, signed, full_range, fuzzable, name)
